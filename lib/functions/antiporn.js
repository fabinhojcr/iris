...
exports.init = async (k, m) => {
	...
	try {
		...
		if (!isGroupMsg || isGroupMsg && !functions.antiporn.includes(chatId) || type == 'ptt' || config.Check_Stickers == false && type == 'sticker' || !isBotGroupAdmins || isOwner || isGroupAdmins || fromMe) return true;
			...
			switch (type) {
				...
				case 'sticker':
					...
					if (needs_ban !== true) {
						...
						let Upload_Image = await tools('cloud').upload(Fixed_Image);
