...
exports.init = async (k, m) => {
	...
	try {
		...
		if (isGroupMsg && !functions.antitravas.includes(chatId) || type == 'ptt' || config.Check_Stickers == false && type == 'sticker' || !isBotGroupAdmins || isOwner || isGroupAdmins || fromMe) return true;
		...
		const Quoted_notifyName = quotedMsgObj ? quotedMsgObj.sender.pushname || quotedMsgObj.notifyName || false : '';
		...
		Quoted_Message_Texts = Message_Texts.concat(Quoted_Caption, Quoted_Footer, Quoted_VCFs_Name, Quoted_VCFs_Content, Quoted_Text, Quoted_list_title, Quoted_list_description, Quoted_list_buttonText, Quoted_list_footerText);
