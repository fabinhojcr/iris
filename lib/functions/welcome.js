...
exports.welcomer = async (kill, events) => {
	...
	try {
		...
		// Configurações
		const irisNumber = (await kill.getHostNumber()) + '@c.us';
		const isIris = events.who == irisNumber;
		const isBlacklist = functions.blacklist.includes(events.chat) && functions.antinumbers.some(uz => events.who.startsWith(uz)); // functions.blacklist.includes(events.chat) && functions.antinumbers.includes(events.who)
		...
		if (events.action == 'add' && isBlacklist && !isIris) {
			...
			if (Open_Welcome.time < Date.now()) {
				...
				if (!Object.keys(customMsg[events.chat]).includes('welcome')) {
					const profile = await tools('profile').getProfilePic(kill, false, false, false, false, [events.who, events.chat], {id:events.who}, irisNumber, groupInfo.groupMetadata.participants, 1);
					try {
				...
				} else await kill.sendTextWithMentions(events.chat, `Olá @${events.who.replace('@c.us', '')}!\n` + customMsg[events.chat].welcome.message);
			} else {
			...
			if (Open_Goodbye.time < Date.now()) {
				...
				if (!Object.keys(customMsg[events.chat]).includes('goodbye')) {
					const profolo = await tools('profile').getProfilePic(kill, false, false, false, false, [events.who, events.chat], {id:events.who}, irisNumber, groupInfo.groupMetadata.participants, 1);
					try {
				...
				} else await kill.sendTextWithMentions(events.chat, `Adeus @${events.who.replace('@c.us', '')}!\n` + customMsg[events.chat].goodbye.message);
			} else {
