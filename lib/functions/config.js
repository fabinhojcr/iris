		...
		for (let i = 0; i < multiCommand.length; i++) {
			...
			switch (cmd) {
				...
				case 'del':
					if (!quotedMsgObj) return await kill.reply(chatId, mess.nomark(), id);
					if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner || isGroupMsg && Can_Run_CMD(cmd)) {
						if (!quotedMsgObj.fromMe && !isBotGroupAdmins) return await kill.reply(chatId, mess.mymess(), id);
				...
				case 'ytsearch':
					...
					if (videoFinded.all.length == 0) return await kill.reply(chatId, mess.noresult(), id);
					...
					await kill.sendYoutubeLink(chatId, `${videoFinded.all[0].url}`, '\n' + mess.play(videoFinded));
					...
					for (let prop of Object.keys(commandForm)) {
						...
						await kill.awaitMessages(chatId, filtercmd, {
							...
							if (/cancel/gi.test(removeAccents(Array.from(collected)[0][1].text))) {
								...
								newCmdToAdd[prop] = Array.from(collected)[0][1].text
							}
				...
				case 'mix':
					...
					if (argl[0] == '-new' || argl[0] == null) {
						...
						kill.awaitMessages(chatId, Mix_Filter, {
							...
							Mix_Waiting[chatId]--;
							placar[m_bdy.sender.id] = placar[m_bdy.sender.id] == null || isNaN(placar[m_bdy.sender.id]) ? 1 : (placar[m_bdy.sender.id] + 1);
